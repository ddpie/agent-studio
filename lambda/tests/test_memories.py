...
def mock_ws_table():
    with patch("crud.memories._get_workspaces_table") as g:
        t = MagicMock()
        t.get_item.return_value = {"Item": {"workspaceId": "ws1", "sk": "META", "memory_id": "mem-abc"}}
        g.return_value = t
        yield t
...
def test_list_initial_load_four_sections(mock_agentcore_data, mock_ws_table):
    ...
    mock_agentcore_data.list_memory_records.return_value = {
        "memoryRecordSummaries": [
            {
                "memoryRecordId": "r1",
                "content": {"text": "prefers Chinese"},
                "createdAt": 1714000000,
                "namespace": "/users/a_u/preferences/",
            }
        ],
        "nextToken": None,
    }
    result = _list_my_memories_impl(
        workspace_id="ws1", agent_id="a", caller_id="u", strategy=None, next_token=None
    )
...
def test_list_uses_actor_id(mock_agentcore_data, mock_ws_table):
    ...
    mock_agentcore_data.list_memory_records.return_value = {"memoryRecordSummaries": [], "nextToken": None}
    _list_my_memories_impl(
        workspace_id="ws1", agent_id="agent-X", caller_id="user-Y", strategy="preferences", next_token=None
    )
...
def test_list_single_strategy_pagination(mock_agentcore_data, mock_ws_table):
        ...
        "memoryRecordSummaries": [
            {
                ...
                "content": {"text": f"f{i}"},
                "createdAt": 1714000000 + i,
                "namespace": "/users/a_u/facts/",
            }
            for i in range(50)
        ],
    ...
    result = _list_my_memories_impl(
        workspace_id="ws1", agent_id="a", caller_id="u", strategy="facts", next_token=None, max_results=50
    )
...
def test_list_missing_memory_id_returns_empty(mock_agentcore_data, mock_ws_table):
    ...
    mock_ws_table.get_item.return_value = {"Item": {"workspaceId": "ws1", "sk": "META"}}
    result = _list_my_memories_impl(
        workspace_id="ws1", agent_id="a", caller_id="u", strategy=None, next_token=None
    )
    assert result == {}
...
def test_list_unknown_strategy_raises(mock_agentcore_data, mock_ws_table):
    ...
    with pytest.raises(ValueError):
        _list_my_memories_impl(
            workspace_id="ws1", agent_id="a", caller_id="u", strategy="bogus", next_token=None
        )
...
def test_delete_record_success(mock_agentcore_data, mock_ws_table):
    ...
    _delete_record_impl(workspace_id="ws1", agent_id="agent-X", caller_id="user-Y", record_id="mem-123")
...
def test_delete_record_cross_user_silent_noop(mock_agentcore_data, mock_ws_table):
    ...
    _delete_record_impl(workspace_id="ws1", agent_id="agent-X", caller_id="user-Y", record_id="mem-OTHER")
...
def test_delete_record_cross_agent_silent_noop(mock_agentcore_data, mock_ws_table):
    ...
    _delete_record_impl(workspace_id="ws1", agent_id="agent-X", caller_id="user-Y", record_id="mem-1")
    mock_agentcore_data.delete_memory_record.assert_not_called()
...
def test_delete_record_already_deleted_idempotent(mock_agentcore_data, mock_ws_table):
    ...
    # Must not raise.
    _delete_record_impl(workspace_id="ws1", agent_id="agent-X", caller_id="user-Y", record_id="mem-x")
...
def test_forget_all_under_limit(mock_agentcore_data, mock_ws_table):
    ...
    mock_agentcore_data.list_memory_records.return_value = {
        "memoryRecordSummaries": [
            {"memoryRecordId": f"r{i}", "namespace": "/users/a_u/preferences/"} for i in range(5)
        ],
        "nextToken": None,
    }
...
def test_forget_all_over_limit(mock_agentcore_data, mock_ws_table):
    ...
    mock_agentcore_data.list_memory_records.return_value = {
        "memoryRecordSummaries": [
            {"memoryRecordId": f"r{i}", "namespace": "/users/a_u/preferences/"} for i in range(300)
        ],
        "nextToken": None,
    }
    mock_agentcore_data.delete_memory_record.return_value = {}
...
def test_delete_record_no_memory_id(mock_agentcore_data, mock_ws_table):
    ...
    mock_ws_table.get_item.return_value = {"Item": {"workspaceId": "ws1", "sk": "META"}}
    _delete_record_impl(workspace_id="ws1", agent_id="a", caller_id="u", record_id="r1")
    mock_agentcore_data.list_memory_records.assert_not_called()
    mock_agentcore_data.delete_memory_record.assert_not_called()
...
def test_delete_record_pagination_until_found(mock_agentcore_data, mock_ws_table):
    ...
    mock_agentcore_data.list_memory_records.side_effect = [
        {"memoryRecordSummaries": [{"memoryRecordId": "other"}], "nextToken": "page-2"},
        {"memoryRecordSummaries": [{"memoryRecordId": "r1"}], "nextToken": None},
    ]
    ...
    mock_agentcore_data.delete_memory_record.return_value = {}
    _delete_record_impl(workspace_id="ws1", agent_id="a", caller_id="u", record_id="r1")
    mock_agentcore_data.delete_memory_record.assert_called_once_with(memoryId="mem-abc", memoryRecordId="r1")
...
def test_delete_record_with_strategy_filter(mock_agentcore_data, mock_ws_table):
    ...
    mock_agentcore_data.exceptions.ResourceNotFoundException = _RNF
    mock_agentcore_data.delete_memory_record.return_value = {}
    _delete_record_impl(workspace_id="ws1", agent_id="a", caller_id="u", record_id="r1", strategy="facts")
...
def test_forget_all_pagination(mock_agentcore_data, mock_ws_table):
    ...
    pages = [
        ...
        {"memoryRecordSummaries": [{"memoryRecordId": "r3"}], "nextToken": None},
...
def _mock_viewer(workspace_id, user_id):
    member = {"workspaceId": workspace_id, "sk": f"MEMBER#{user_id}", "userId": user_id, "role": "viewer"}
...
def test_route_list_unknown_strategy_400(
    ...
    resp = _invoke(
        _apigw(
            "GET",
            f"/api/workspaces/{workspace_id}/agents/agt1/my-memories",
...
def test_route_list_no_membership_403(workspace_id, mock_jwt, _mock_no_membership):
    resp = _invoke(_apigw("GET", f"/api/workspaces/{workspace_id}/agents/agt1/my-memories"))
...
def test_route_delete_record_success(
    ...
    mock_agentcore_data.list_memory_records.return_value = {
        "memoryRecordSummaries": [{"memoryRecordId": "rec-1"}],
        "nextToken": None,
    }
...
def test_route_delete_record_unknown_strategy_400(
    ...
    resp = _invoke(
        _apigw(
            "DELETE",
            f"/api/workspaces/{workspace_id}/agents/agt1/my-memories/rec-1",
...
def test_route_delete_record_forbidden(
    ...
    with patch("crud.memories._delete_record_impl", side_effect=MemoryForbidden("nope")):
        resp = _invoke(_apigw("DELETE", f"/api/workspaces/{workspace_id}/agents/agt1/my-memories/rec-1"))
    assert resp["statusCode"] == 403


def test_route_delete_record_no_membership_403(workspace_id, mock_jwt, _mock_no_membership):
    resp = _invoke(_apigw("DELETE", f"/api/workspaces/{workspace_id}/agents/agt1/my-memories/rec-1"))
    assert resp["statusCode"] == 403


def test_route_forget_all_success(workspace_id, mock_jwt, _mock_viewer, mock_agentcore_data, mock_ws_table):
    mock_agentcore_data.list_memory_records.return_value = {
        "memoryRecordSummaries": [],
        "nextToken": None,
    }
...
def test_route_forget_all_handles_unexpected_error(workspace_id, mock_jwt, _mock_viewer):
    with patch("crud.memories._forget_all_impl", side_effect=RuntimeError("boom")):
        resp = _invoke(_apigw("DELETE", f"/api/workspaces/{workspace_id}/agents/agt1/my-memories"))
    assert resp["statusCode"] == 500


def test_route_forget_all_no_membership_403(workspace_id, mock_jwt, _mock_no_membership):
    resp = _invoke(_apigw("DELETE", f"/api/workspaces/{workspace_id}/agents/agt1/my-memories"))
    assert resp["statusCode"] == 403


def test_route_list_handles_value_error_from_impl(workspace_id, mock_jwt, _mock_viewer):
    """_list_my_memories_impl raising ValueError should map to 400."""
    with patch("crud.memories._list_my_memories_impl", side_effect=ValueError("bad strategy from impl")):
        resp = _invoke(_apigw("GET", f"/api/workspaces/{workspace_id}/agents/agt1/my-memories"))
    assert resp["statusCode"] == 400
