...
class TestGetMcpPolicy:
    def test_default_policy(self, workspace_id, mock_jwt, _mock_editor, mock_ws_table):
    ...
    def test_existing_policy_returned(self, workspace_id, mock_jwt, _mock_editor, mock_ws_table):
    ...
    def test_no_membership_forbidden(self, workspace_id, mock_jwt, _mock_no_membership, mock_ws_table):
        resp = _invoke(_apigw("GET", f"/api/workspaces/{workspace_id}/mcp/policy"))
...
class TestUpdateMcpPolicy:
    def test_admin_updates(self, workspace_id, mock_jwt, _mock_admin, mock_ws_table):
        ...
        resp = _invoke(_apigw("PUT", f"/api/workspaces/{workspace_id}/mcp/policy", body=body))
        assert resp["statusCode"] == 200
        data = json.loads(resp["body"])
        assert data["policy"]["mode"] == "allowlist"
        # Update was called
    ...
    def test_editor_forbidden(self, workspace_id, mock_jwt, _mock_editor, mock_ws_table):
        body = {"policy": {"mode": "all"}}
        resp = _invoke(_apigw("PUT", f"/api/workspaces/{workspace_id}/mcp/policy", body=body))
        assert resp["statusCode"] == 403

    def test_invalid_mode(self, workspace_id, mock_jwt, _mock_admin, mock_ws_table):
        body = {"policy": {"mode": "invalid"}}
        resp = _invoke(_apigw("PUT", f"/api/workspaces/{workspace_id}/mcp/policy", body=body))
        assert resp["statusCode"] == 400
        assert "mode" in json.loads(resp["body"])["error"]

    def test_missing_lists_added(self, workspace_id, mock_jwt, _mock_admin, mock_ws_table):
        body = {"policy": {"mode": "all"}}
        resp = _invoke(_apigw("PUT", f"/api/workspaces/{workspace_id}/mcp/policy", body=body))
        assert resp["statusCode"] == 200
        data = json.loads(resp["body"])
...
class TestListAvailableTargets:
    def test_filtered_for_member(self, workspace_id, mock_jwt, _mock_editor, mock_ws_table):
        targets = [
        ...
        with patch("crud.mcp._build_target_list", return_value=targets):
            ...
            resp = _invoke(_apigw("GET", f"/api/workspaces/{workspace_id}/mcp/targets"))
        assert resp["statusCode"] == 200
        data = json.loads(resp["body"])
        names = [t["name"] for t in data["items"]]
        assert names == ["a"]

    def test_show_all_admin_only(self, workspace_id, mock_jwt, _mock_admin, mock_ws_table):
        targets = [{"name": "a"}, {"name": "b"}]
        with patch("crud.mcp._build_target_list", return_value=targets):
            mock_ws_table.get_item.return_value = {
                "Item": {"mcpPolicy": {"mode": "allowlist", "allowedTargets": ["a"]}}
    ...
    def test_show_all_editor_forbidden(self, workspace_id, mock_jwt, _mock_editor, mock_ws_table):
        resp = _invoke(
            _apigw(
                "GET",
                f"/api/workspaces/{workspace_id}/mcp/targets",
...
class TestGetTargetTools:
    ...
    def test_no_membership_forbidden(self, workspace_id, mock_jwt, _mock_no_membership):
        resp = _invoke(
            _apigw(
                "GET",
                f"/api/workspaces/{workspace_id}/mcp/targets/x/tools",
