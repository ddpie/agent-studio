/**
 * Tool Assistant — AI sidebar for editing tool code via natural language.
 * Modeled after SkillAssistant.tsx.
 */
import { useState, useRef, useEffect, memo, useCallback } from "react";
import { useTranslation } from "react-i18next";
import { useToolAssistantStore, type ToolAssistantMessage } from "../../stores/tool-assistant-store";
...
export default function ToolAssistant({ toolId, currentCode, toolName, toolDescription, toolCategory, onCodeUpdate }: ToolAssistantProps) {
  const { t } = useTranslation();
  const {
  ...
  const handleSend = () => {
    ...
    setInput("");
    sendMessage(text, { name: toolName, description: toolDescription, category: toolCategory, code: currentCode }, onCodeUpdate);
  };
  ...
  const submitEdit = () => {
    ...
    sendMessage(newContent, { name: toolName, description: toolDescription, category: toolCategory, code: currentCode }, onCodeUpdate);
  };
      ...
      <div className="border-t border-gray-100 dark:border-gray-700 px-3 py-2">
        <div className="flex items-end gap-2">
          <textarea
            ...
            placeholder={t("toolEditor.describeChange")}
            rows={1}
          ...
          />
          {isStreaming ? (
            <button onClick={cancelStreaming} className="p-1.5 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors" title={t("common.stop")}>
