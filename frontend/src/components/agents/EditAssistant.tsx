    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-2 group animate-[fadeSlideIn_0.2s_ease-out]`}>
      ...
      >
        ...
        {!isUser && isLastAssistant && isStreaming && msg.content && (
          <div className="flex items-center justify-between mt-2 pt-1.5 border-t border-gray-200 dark:border-gray-700 text-[11px] text-blue-500 animate-pulse">
            <span className="flex items-center gap-2">
              <Loader2 className="w-3 h-3 animate-spin" />
            ...
            <button
              onClick={() => onShowPreview?.()}
              className="flex items-center gap-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-800 rounded text-[10px] font-medium transition-colors animate-none text-blue-600 dark:text-blue-400"
            >
                  ...
                  <div key={group.label}>
                    ...
                    {group.models.map((m) => (
                      <button
                        ...
                        className={`w-full text-left px-2 py-1 text-[10px] hover:bg-blue-50 dark:hover:bg-blue-900/30 ${selectedModelId === m.id ? "text-blue-600 dark:text-blue-400 bg-blue-50/50 dark:bg-blue-900/20" : "text-gray-700 dark:text-gray-300"}`}
                      >
                        {m.label}
                      </button>
                    ))}
                ...
                <div key={msg.id} className="flex justify-end mb-2">
                  <div className="max-w-[90%] w-full">
                    <textarea
                      ...
                      autoFocus
                      rows={2}
                      className="w-full px-2 py-1.5 rounded-lg border-2 border-blue-400 dark:border-blue-500 text-xs focus:outline-none resize-none bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                    />
        ...
        {messages.length > 0 && !isStreaming && messages[messages.length - 1]?.role === "assistant" && messages[messages.length - 1]?.content && (
          <div className="flex justify-start">
            <button
              ...
              className="flex items-center gap-1 text-[10px] text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
            >
