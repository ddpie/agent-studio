...
export default function SecretsTab({ agentId }: Props) {
  ...
  async function onDelete(key: string) {
    if (!confirm(t("secrets.confirmDelete", { name: key }))) return;
    try {
  ...
  return (
    <div className="p-4" data-testid="secrets-tab">
      ...
      {error && (
        <div className="text-sm text-red-600" data-testid="secrets-error">
          {error.message}
        </div>
      )}
      ...
      {secrets && secrets.length === 0 && !loading && (
        <div className="text-sm text-gray-500" data-testid="secrets-empty">
          {t("secrets.empty")}
        </div>
      )}
    ...
    >
      ...
      >
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          <div>
            <label className="block text-xs text-gray-500 mb-1" htmlFor="secret-key">
              {t("secrets.keyLabel")}
            </label>
            <input
              id="secret-key"
              ...
              spellCheck={false}
            />
            <div className="text-[11px] text-gray-500 mt-1">{t("secrets.keyHint")}</div>
            {keyErr && (
              <div className="text-xs text-red-600 mt-1" data-testid="secret-key-error">
                {keyErr}
              </div>
            )}
            {isOverwrite && !keyErr && (
              <div className="text-xs text-amber-600 mt-1" data-testid="secret-overwrite-warning">
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1" htmlFor="secret-value">
            ...
            <div className="relative">
              <input
                ...
                className="w-full text-sm px-2 py-1.5 pr-8 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950"
            ...
            {valueErr && (
              <div className="text-xs text-red-600 mt-1" data-testid="secret-value-error">
                {valueErr}
              </div>
            )}
          </div>
