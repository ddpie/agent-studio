...
def update_agent(
    ...
    The call runs synchronously: zip reassembly + S3 upload + the
    update_agent_runtime API call all happen before this tool returns
    (typically 40-80s). Meta-Agent's stream keepalive (15s tick) keeps
    CloudFront's 60s origin idle timeout from firing while we wait.

    An earlier version of this tool detached the heavy work into a daemon
    thread and returned ``QUEUED`` immediately. That silently lost the
    deploy whenever AgentCore recycled the container right after the tool
    returned — the thread died mid-zip-upload and the user saw
    metadata.json bump while deployment.zip stayed at the old mtime. Now
    we keep everything on the main task so the tool only returns after
    ``update_agent_runtime`` has been accepted by the control plane.
    ...
    # tool_definitions / tool_names MUST fall back to existing metadata.
    # Before 2026-05-06, an update_agent call that didn't pass these fields
    # (e.g. a prompt-only tweak) silently cleared them, wiping the
    # `call_agent` tool that link_agent had injected — the link relationship
    # survived in DDB linked_agents but the tool itself was gone from the
    ...
    if _existing_tool_names is None:
        _existing_tool_names = existing_metadata.get("tools") or []
    if isinstance(_existing_tool_names, list):
        _existing_tool_names_str = ",".join(_existing_tool_names)
    else:
        _existing_tool_names_str = str(_existing_tool_names or "")
    final_tools_names = tool_names or _existing_tool_names_str
    ...
    # Ensure BASE_GUIDELINES is present on the persisted prompt. We use
    # substring containment to stay idempotent across repeated updates —
    # if the exact bytes are already in the prompt from a previous deploy
    # we skip. Language is picked from the creator's UI locale (set in
    # tools._scope from the invoke payload). When older agents
    ...
    # Inject built-in tool code for tools named in tool_names but not
    ...
    prompt_txt = final_prompt + skill_prompt_section if skill_prompt_section else final_prompt
