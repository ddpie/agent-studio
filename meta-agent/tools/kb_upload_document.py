...
def kb_upload_document(kb_id: str, staging_key: str, filename: str) -> str:
            ...
            return json.dumps(
                {
                    ...
                    "max_mb": 50,
                    "actual_mb": round(head["ContentLength"] / 1024 / 1024, 1),
                }
            )
    except Exception as e:
    ...
    try:
        s3.copy_object(
            Bucket=S3_BUCKET, CopySource={"Bucket": S3_BUCKET, "Key": staging_key}, Key=document_key
        )
    except Exception as e:
    ...
    try:
        ...
        return json.dumps(
            {"error": "ingestion_start_failed", "message": str(e), "document_key": document_key}
        )
