...
if [[ -z "${ORIGIN_VERIFY_SECRET:-}" ]]; then
  ...
  # Persist immediately — use grep to avoid duplicates on repeated runs
  if [[ -f "$PROJECT_ROOT/.env" ]] && grep -q "^ORIGIN_VERIFY_SECRET=" "$PROJECT_ROOT/.env"; then
    sed -i.bak "s/^ORIGIN_VERIFY_SECRET=.*/ORIGIN_VERIFY_SECRET=$ORIGIN_VERIFY_SECRET/" "$PROJECT_ROOT/.env" && rm -f "$PROJECT_ROOT/.env.bak"
  else
    echo "ORIGIN_VERIFY_SECRET=$ORIGIN_VERIFY_SECRET" >> "$PROJECT_ROOT/.env"
  fi
fi
...
# Generated by deploy-all.sh — $(date -u +"%Y-%m-%dT%H:%M:%SZ")
AGENT_STUDIO_REGION=$REGION
AGENT_STUDIO_ACCOUNT_ID=$ACCOUNT_ID
AGENT_STUDIO_META_AGENT_ID=$META_AGENT_ID
AGENT_STUDIO_COGNITO_USER_POOL_ID=$USER_POOL_ID
AGENT_STUDIO_COGNITO_CLIENT_ID=$CLIENT_ID
AGENT_STUDIO_S3_BUCKET=$S3_BUCKET
AGENT_STUDIO_CLOUDFRONT_DOMAIN=$CF_DOMAIN
AGENT_STUDIO_API_URL=https://$CF_DOMAIN
AGENT_STUDIO_FRONTEND_BUCKET=$FRONTEND_BUCKET
AGENT_STUDIO_CLOUDFRONT_ID=$CF_ID
